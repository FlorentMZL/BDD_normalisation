...
public class EGD extends Constraint {
     ...
     * @param head
     */
    public EGD(final List<List<String>> body, final Set<String> equalities, final Set<String> head) {
        this.body = body;
    ...
    public boolean apply(Database database) {
        ...
        if (this.currentNumberOfRecords == -1) { // -1 means that the constraint has never been applied
            ...
            for (var tableName : this.body){
                System.out.println(tableName.get(0));
             allRecords.addAll(database.getTable(Database.stripNumerals(tableName.get(0))).getRecords()); // Push all relevant records into a set
            }
        ...
        for (var equality : this.equalities) { 
            ...
            for (var record1 : allRecords) {
                for (var record2 : allRecords) {
                    ...
                    for(var l : this.body){
                        if(Database.stripNumerals(l.get(0)).equals(record1.getTable())){
                            if (l.contains(firstAttribute)){
                                for(var l2 :this.body){
                                    if (Database.stripNumerals(l2.get(0)).equals(record2.getTable())&&!l2.equals(l)){
                                        if(l2.contains(secondAttribute)){
        ...
        for (var equality : this.head) {
            ...
            for (var matchingRecord1 : matchingRecords) { // Parse the matching records
                for (var matchingRecord2 : matchingRecords) { // This is a double loop because we need to compare every record to every other record
                    ...
                    for(var l : this.body){
                        if(Database.stripNumerals(l.get(0)).equals(matchingRecord1.getTable())){
                            if (l.contains(firstAttribute)){
                                for(var l2 :this.body){
                                    if (Database.stripNumerals(l2.get(0)).equals(matchingRecord2.getTable())&&!l2.equals(l)){
                                        if(l2.contains(secondAttribute)){ // Only compare records that have the right attributes
